# 26-05-14 HTML, CSS, JS

1. input-field에서 field를 사용하는 이유
   - field = 입력 공간
   - input = html 태그 이름, input-field = 입력 공간 스타일

2. let = 나중에 값이 바뀔 수 있는 변수, const = 한 번 정하면 절대 안 바뀌는 변수
...
11. `async function(){ const response = await fetch(url) }`
...
loadpostBtn.addEventListener('click', async function(){
    console.log('버튼 클릭!')
    result.textContent = '데이터를 가져오는 중입니다. 조금만 기다려 주세요.'
...
15. document.createElement
    ...
    - html의 article(기호가 없으니 태그 선택자)를 js에서 새로 만드는 것 ; 영화의 수를 모르니까 html에 미리 못 씀 -> js가 그때그때 article 태그를 새로 만듦
