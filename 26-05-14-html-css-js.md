input-field에서 field를 사용하는 이유
field = 입력 공간
input = html 태그 이름, input-field = 입력 공간 스타일

let = 나중에 값이 바뀔 수 있는 변수, const = 한 번 정하면 절대 안 바뀌는 변수
ex) let은 나이, const는 이름 / let은 inputValue, const는 loginBtn (로그인 버튼은 안 바뀌니까)
변수 vs 상수, const도 변수인가?
엄밀히는 let = 변수, const = 상수
일상 대화(실무)에서는 const도 "변수"라고 부름
Python 2강에서 변수를 “데이터를 담아두는 상자”라고 칭함
JS에서도 const loginBtn = document.querySection(’#loginBtn’)
: loginBtn = 버튼을 담아두는 상자

querySelector(getElementBy... 대신 사용)를 사용하면 요소 구별 방법
뒤에 오는 기호로 구별 (ex. #, . , 기호 없음) = CSS와 동일
#: id 선택자 (페이지에 하나만 존재), .: class 선택자 (여러 개 동시 존재 가능), 기호 없음: 태그 선택자 (그 태그 전부 선택)

if(loginBtn): loginBtn이 진짜 존재하면 (loginBtn이 null이 아니면)
사용 이유: html에 로그인 버튼이 없는 페이지 (ex. 회원가입)에서 이 js가 실행되면 에러 발생

addEventListener 정확한 역할 및 예시
...
section과 div로 구별해서 사용한 이유


section은 해당 구역 전체를 나타내며 타 section과 구별이 가능함
div는 그저 해당 내용(결과)을 채울 빈 공간에 불과
section은 구역, div는 그릇 같은 느낌


querySelector를 사용했을 때 # 기호를 사용한 이유

loadPostBtn, result는 각 페이지에 하나밖에 없는 고유한 값이라 id이기 때문에 (#)

result가 let이 아닌 const를 사용한 이유
result는 값의 결과값이 아닌 result가 나오는 상자, 공간
상자 자체(const)는 안 바뀜, 상자 안 내용(result.innerHTML)은 바뀜

API 흐름
사용자 → 버튼 클릭
JS → fetch()로 api 서버에 요청 ('이 url로 다녀와 줘.'라는 부탁 / 결과가 '언젠가 도착할' 작업을 비동기(async)로 처리)
...
async function(){ const response = await fetch(url) }
function 뒤의 함수 안에서 await 줄에서 잠깐 대기
함수 바깥에 있는 다른 것들은 계속 돌아감
url에서 응답이 오면 다음 줄 실행


response와 response.json()
loadpostBtn.addEventListener('click', async function(){ console.log('버튼 클릭!') result.textContent = '데이터를 가져오는 중입니다. 조금만 기다려 주세요.' 

const response = await fetch('url link') console.log('response:', response) 

// 서버 응답은 바로 사용할 수 없기 때문에 json()형식으로 바꿔야 함 
const data = await response.json() 
** response.json() = json 형식으로 변환해서 꺼내 줘. 
...
13. `` (백틱, backtick): 문자열 안에 변수를 끼워넣을 때 사용
   * 기존 방식(""): "<h2>" + 제목 + "</h2><p>개봉일: " + 날짜 + "</p>"
   * 백틱 방식(``): `<h2>${제목}</h2><p>개봉일: ${날짜}</p>`
...
15. document.createElement
   ...
   * html의 article(기호가 없으니 태그 선택자)를 js에서 새로 만드는 것 ; 영화의 수를 모르니끼 html에 미리 못 씀 -> js가 그때그때 article 태그를 새로 만듦
   * const card = 만든 article 태그를 card라는 변수에 담기
   * Elements가 아닌 Element인 이유: 태그를 딱 하나만 만드니까 / 여러 개를 만들고 싶으면 forEach 사용 
   * movies.forEach(function(movie){
               const card = document.createElement("article")
               movieList.appendChild(card) / 어쨌든 태그를 만드는 건 단수 행위이고, forEach가 반복하는 거니까 s는 오지 않음
...
18. * fetch(TMDB_URL, {
   ...
   * headers: 요청할 때 함께 보내는 부가 정보(봉투 겉면) / '이제부터 부가 정보를 말할 거야~'라고 암시하는 단어라고 생각
   * accept:"application/json": json 형식으로 데이터를 받고 싶음
   * Authorization: Bearer ${TMDB_TOKEN}: 영화 데이터 서비스(TMDB)를 회원증(TOKEN)을 보여 주고 증명하는 것
